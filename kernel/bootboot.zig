...
const ProtocolLevel = enum(u2) {
    MINIAMAL = 0, STATIC = 1, DYNAMIC = 2
};

const Endianess = enum(u1) {
    BIG_ENDIAN = 1, LITTL_ENDIAN = 0
};

const LoaderType = enum(u5) {
    BIOS = (0 << 2), UEFI = (1 << 2), RASPBERRY_PI = (2 << 2)
};
...
const PlatformX8664 = extern struct {
    acpi: u64, smbi: u64, efi: u64, mp: u64, unused: [4]u64
};

const PlatformAarch64 = extern struct {
    acpi: u64, mmio: u64, efi: u64, unused: [5]u64
};

const ArchSpecific = extern union {
    x8664: PlatformX8664, aarch64: PlatformAarch64
};
